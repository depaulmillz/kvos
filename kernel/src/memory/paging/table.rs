...
use super::entry::{ENTRY_COUNT, EntryFlags, Entry};
        ...
        where A : FrameAllocator
    {
        if self.next_table(index).is_none() {
            assert!(!self.entries[index].flags().contains(EntryFlags::HUGE_PAGE),
                    "do not support mapping huge pages");
            let frame = allocator.allocate_frame().expect("no frames available");
            self.entries[index].set(frame.clone(), EntryFlags::PRESENT | EntryFlags::WRITABLE | EntryFlags::USER_ACCESSIBLE); 
                                                                                  // TODO added
                                                                                  // user
                                                                                  // accessible
                                                                                  // hack
